...
## 10. Prayer widgets — BUILT & WIRED ✅ (full suite)
**Status (2026-06-17):** `DuhaaWidgets` extension target created + wired in `project.pbxproj`; **app + widget build for the Simulator**, `.appex` embeds, **App Groups (`group.com.duhaa.app`) on both targets** (verified in `*-Simulated.xcent` + runtime container resolves), **241 tests pass**.
- **10 widgets in the bundle.** Home (color): Next Prayer (small), Today's Prayers (medium, interactive), Prayer Day (large, interactive), 7-Day Grid (small), Daily Du'a (medium). Lock Screen (monochrome): Next Prayer **Countdown** (circ+rect, `Text(timerInterval:)` + ring stepped by `PrayerTimelinePlanner.ringStepDates`), **Prayer Tracker** (circ+rect, each prayer **tappable** via `TogglePrayerCompletionIntent`), **Morning**/**Evening** times (rect+circ), **Hijri Date** (circ+inline), 7-Day Grid (accessory).
- **Five-ready state model:** `PrayerState` enum (in `PrayerWidgetShared.swift`) has all 5 cases + `upcoming`, but `derive(...)` only ever produces on-time/late/missed/upcoming from real data — made-up/excused stay reserved (no fabrication, no core-tracker change). Monochrome via `PrayerState.monoSymbol`/`gridSymbol`; Home color via `WidgetTheme.color(for:)`.
- **App writes** sunrise + Hijri (offset-aware) + today's du'a into the shared payload (`WidgetSnapshotWriter`); du'a tap deep-links `duhaa://dua/<index>` → Du'as tab (`DuhaaApp.onOpenURL`).
- Remaining: device-only App Groups provisioning (`docs/widgets-setup.md` Device note) + manual QA (`docs/widgets-qa.md`).
- **What exists:** `Duhaa/Shared/` (App-Group store + `PrayerID` + `PrayerTimesPayload` + `PrayerWidgetSnapshot` builder + `SetPrayerCompletionIntent`/`TogglePrayerCompletionIntent` + `PrayerTimelinePlanner`); `Duhaa/Support/WidgetSnapshotWriter.swift` (app-only, Adhan-backed payload writer); `DuhaaWidgets/` (bundle + small "Next Prayer" + medium "Today's Prayers" (interactive Toggles) + large "Prayer Day" (interactive) + Lock Screen accessories + theme bridge + components, with `#Preview`s).
- **Storage:** `PrayerTracker` now persists to `UserDefaults.duhaaShared` (App-Group suite `group.com.duhaa.app`) with a one-time `.standard`→suite migration (flag `duhaa.shared.migratedTrackerV1`). Same keys/shape as before, so widget + app share completion automatically. App→widget: `WidgetCenter.reloadAllTimelines()` after each completion change + on payload refresh. Widget→app: `tracker.reloadFromStore()` on foreground.
- **Theme parity:** widgets reuse `Palette.swift` (`AppTheme.colors`) for exact Classic + Light Pink colors. Visual mockup was rendered & reviewed.
